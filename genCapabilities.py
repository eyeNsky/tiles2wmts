...
def footerTemplate():
  ...
  <ServiceMetadataURL xlink:href="%(link)s"/>
</Capabilities>"""
  return footer
# get json
...
jin.close()
# set up output
xmlOut = open('WMTSGetCapabilities.xml','w')
args = {}
args['title'] = tjson['title']
args['link'] = tjson['link']
layers = ''
for js in tjson['tilejson']:
  ...
  #print theTiles
	...
	layers += layerTemplate()%args
# set up output
xmlOut = open('WMTSGetCapabilities.xml','w')
xmlOut.write(headerTemplate()%args)
xmlOut.write(layers)
xmlOut.write(footerTemplate()%args)
xmlOut.close()
